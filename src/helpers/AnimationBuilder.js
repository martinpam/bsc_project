...
function addMoveAnimation(step, animations, counters, updates) {
    ...
    const fadeOut = sock.animate([{
        opacity: 0
    }], {
        duration: 500,
        fill: 'forwards'
    });
    const fadeIn = sock.animate([{
        opacity: 1
    }], {
        duration: 300,
        fill: 'forwards'
    });
    const fillerEnd = sock.animate([{}], {
        duration: 0
    });
    const filler = sock.animate([{}], {
        duration: 0
    });
...
function addSelectAnimation(step, animations) {
    const sock = document.getElementById(step.sock.identifier)
    const selection = sock.firstChild.animate([{
        width: '114px',
        height: '114px'
    }], {
        duration: 350,
    ...
    const deselection = sock.firstChild.animate([{
        width: '100px',
        height: '100px'
    }], {
        duration: 350,
...
function addSelectAllAnimation(step, animations, select, counters) {
    ...
    let filler = prepSock.animate([{}], {
        duration: 0
    });
    let fillerEnd = prepSock.animate([{}], {
        duration: 0
    });
    const selections = [];
    const deselections = [];
    ...
    for (let i = 0; i < step.socks.length; i++) {
        ...
        selections.push(socks[i].firstChild.animate([{
            width: '114px',
            height: '114px'
        }], {
            duration: 350,
        ...
        deselections.push(socks[i].firstChild.animate([{
            width: '100px',
            height: '100px'
        }], {
            duration: 350,
    ...
    for (let z = 0; z < selections.length; z++) {
        animations.push({
            animation: selections[z],
            last: false
        })
        animations.push({
            animation: deselections[z],
            last: false
        })
    }
    animations.push({
...
function addTransferSocksAnimation(step, animations, updates) {
    ...
    let basketAnimation = animationElem.animate([{
        width: '0px'
    }], {
        duration: 600,
        fill: 'forwards'
    })
    let basketAnimationRevert = animationElem.animate([{
        width: '150px'
    }], {
        duration: 600,
        fill: 'forwards'
    ...
    animations.push({
        ...
        animation: basketAnimationRevert,
        last: false
    }, {
        animation: fillerEnd,
        last: true
...
function addMoveAllAnimation(step, animations, counters, updates) {
    ...
    for (let i = 0; i < step.socks.length; i++) {
        const sock = document.getElementById(step.socks[i].identifier)
        fadeOuts.push(sock.animate([{
            opacity: 0
        }], {
            duration: 500,
            fill: 'forwards'
        }));
        fadeIns.push(sock.animate([{
            opacity: 1
        }], {
            duration: 300,
            fill: 'forwards'
...
function addFinishedAnimation(step, animations) {
    const filler = document.getElementById('app').animate([{}], {
        duration: 0
    });
    const fillerEnd = document.getElementById('app').animate([{}], {
        duration: 0
    });
    filler.onfinish = () => {
